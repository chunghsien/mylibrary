...
abstract class ECPay_Verification
{
    ...
    public function filter_string($arExtend = [], $InvoiceMark = '')
    {
        ...
        foreach ($arExtend as $key => $value) {
            if (! in_array($key, $arPayMentExtend, true)) {
                unset($arExtend[$key]);
            }
        }
    ...
    public function check_invoiceString($arExtend = [])
    {
        ...
        if (! array_key_exists('CarruerType', $arExtend)) {
            ...
            if (strlen($arExtend['CustomerIdentifier']) > 0 && in_array($arExtend['CarruerType'], $notPrint, true)) {
        ...
        if (! array_key_exists('InvoiceItems', $arExtend)) {
            ...
            if ($arExtend['TaxType'] == ECPay_TaxType::Mix) {
                if (in_array(ECPay_TaxType::Dutiable, $tmpItemTaxType, true) && in_array(ECPay_TaxType::Free, $tmpItemTaxType, true)) {
                    // Do nothing
                } else {
                    $tmpItemTaxType = [];
                }
    ...
    private function support_ignore_payment($arParameters = [])
    {
        ...
        return (in_array($arParameters['ChoosePayment'], $arSupportPayments, true) === true);
    }
}
