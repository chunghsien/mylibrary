...
abstract class EcpayInvoiceMethod
{
    ...
    // 延遲或觸發開立發票。
    public const INVOICE_DELAY = 'INVOICE_DELAY';

    // 開立折讓。
    public const ALLOWANCE = 'ALLOWANCE';

    // 線上開立折讓(通知開立)。
    public const ALLOWANCE_BY_COLLEGIATE = 'ALLOWANCE_BY_COLLEGIATE';

    // 發票作廢。
    public const INVOICE_VOID = 'INVOICE_VOID';

    // 折讓作廢。
    public const ALLOWANCE_VOID = 'ALLOWANCE_VOID';

    // 查詢發票。
    public const INVOICE_SEARCH = 'INVOICE_SEARCH';

    // 查詢作廢發票。
    public const INVOICE_VOID_SEARCH = 'INVOICE_VOID_SEARCH';

    // 查詢折讓明細。
    public const ALLOWANCE_SEARCH = 'ALLOWANCE_SEARCH';

    // 查詢折讓作廢明細。
    public const ALLOWANCE_VOID_SEARCH = 'ALLOWANCE_VOID_SEARCH';

    // 發送通知。
    public const INVOICE_NOTIFY = 'INVOICE_NOTIFY';

    // 付款完成觸發或延遲開立發票。
    public const INVOICE_TRIGGER = 'INVOICE_TRIGGER';

    // 手機條碼驗證。
    public const CHECK_MOBILE_BARCODE = 'CHECK_MOBILE_BARCODE';

    // 愛心碼驗證。
    public const CHECK_LOVE_CODE = 'CHECK_LOVE_CODE';
}
...
class EcpayInvoice
{
     ...
     * 版本
     */
    public const VERSION = '1.0.2002102';
        ...
        if (sizeof($arParameters['Items']) == 0) {
            ...
            if (! $bError_Tag) {
                ...
                if ($arParameters['TaxType'] == EcpayTaxType::Mix) {
                    ...
                    if (count($items) != 2 || ! in_array(1, $items, true)) {
