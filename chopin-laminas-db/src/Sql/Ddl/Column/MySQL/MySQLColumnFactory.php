...
abstract class MySQLColumnFactory
{
    ...
    {
        //$column = null;
        if (false !== array_search($type, static::$NumberTextType, true)) {
            return new NumberOrText($name, $type, $options);
        }

        if (false !== array_search($type, static::$TimestampType, true)) {
            return new Timestamp($name, $options);
        }

        if (false !== array_search($type, static::$DecimalType, true)) {
