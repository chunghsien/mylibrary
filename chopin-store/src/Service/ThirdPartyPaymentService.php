...
abstract class ThirdPartyPaymentService
{
     ...
     *
     * @param string $key
     * @param string $phpLang
     * @return []
     */
    protected function getConfig($key, $phpLang)
    {
        // $key = 'newwebpay';
        $systemSettings = $this->systemSettingTableGateway->toSerialize(null, $key);
        $options = $systemSettings[$phpLang]["children"];
        $response = [];
    ...
    protected function saveNotifyLog($data, $post)
    {
        ...
        if ($count == 0) {
            ...
            $pay_log_foder = 'storage/logs/pay';
