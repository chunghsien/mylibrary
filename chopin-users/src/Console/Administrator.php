...
class Administrator extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            $rolesHasPermissionTableGateway = new RolesHasPermissionTableGateway($this->sql->getAdapter());
            foreach ($permissionsResultSet as $permission) {
                ...
                $item = $rolesHasPermissionTableGateway->select($data)->current();
                if ($item) {
                    $rolesHasPermissionTableGateway->update([
                        "is_view" => 1,
                        "is_insert" => 1,
                        "is_edit" => 1,
                        "is_del" => 1,
